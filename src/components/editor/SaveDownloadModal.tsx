...
import { exportToPng, buildPreviewDataUrl } from '@/lib/imageExport'
...
export default function SaveDownloadModal({ cyRef, onClose }: Props) {
  ...
  async function generatePreview() {
    ...
    try {
      const url = await buildPreviewDataUrl({ cy: cyRef.current, title, template, fontStyle })
      setPreview(url)
    } finally {
  ...
  // 開いた時点でプレビュー生成
  useEffect(() => { generatePreview() }, []) // eslint-disable-line react-hooks/exhaustive-deps

  const regeneratePreview = useCallback(() => { generatePreview() }, [cyRef, title, template, fontStyle]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  async function handleSave() {
    ...
    // diagrams INSERT（thumbnail なし - カラム未追加環境での失敗を防ぐ）
    ...
    if (diagErr || !diag) {
      console.error('diagrams insert error:', diagErr)
    ...
    // thumbnail を後付け UPDATE（カラム未追加でも本体保存は保証済み）
    if (cyRef.current) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const thumbnail = (cyRef.current as any).png({ output: 'base64uri', scale: 0.4, full: true, padding: 16 }) as string
      if (thumbnail) {
        const { error: thumbErr } = await supabase.from('diagrams').update({ thumbnail }).eq('id', diag.id)
        if (thumbErr) console.error('thumbnail update error:', thumbErr)
      }
    }
    ...
    <>
      ...
      >
        <div className="w-full max-w-sm mx-4 mb-4 sm:mb-0 rounded-2xl bg-[#1a1a2e] border border-white/10 overflow-hidden shadow-2xl">
          ...
          {/* プレビュー */}
          <div className="relative bg-[#0f0f1a] aspect-video flex items-center justify-center">
            {previewLoading
              ? <p className="text-white/20 text-sm animate-pulse">生成中...</p>
              : preview
              ? <img src={preview} alt="プレビュー" className="w-full h-full object-contain" />
              : <p className="text-white/20 text-sm">プレビューなし</p>
            }
          </div>
          ...
          <div className="p-4 flex flex-col gap-3">
            ...
            <button
              ...
              className="w-full py-2.5 rounded-xl bg-white/8 hover:bg-white/15 border border-white/10 text-white text-sm font-bold disabled:opacity-40 transition-colors"
            >
              {exporting ? '生成中...' : '画像をダウンロード'}
            </button>
